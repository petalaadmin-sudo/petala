...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await admin.rpc('cancel_chat_request', {
    ...
    if (error || !result?.success) {
      if (error) {
        console.error('[/api/chat/cancelar] cancel_chat_request', error)
      }

      return NextResponse.json({
        success: false,
        error: result?.error ?? 'Falha ao cancelar solicitacao',
        code: result?.code ?? 'CANCEL_CHAT_REQUEST_FAILED',
        ...(result ?? {}),
    ...
    console.error('[/api/chat/cancelar]', err)
